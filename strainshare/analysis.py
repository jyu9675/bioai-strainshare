...
Phase 1 keeps the gut<->vagina class names for exact backward-compatibility; the
generalization to arbitrary site pairs lands in Item 4.
"""
import os
...
def analyze(compare, meta, mpa, cfg=None):
    ...
    w = rate(pairs[pairs.pair_class == "within_gut_vagina"]).add_prefix("within_")
    b = rate(pairs[pairs.pair_class == "between_gut_vagina"]).add_prefix("between_")
    ...
    cand = pairs[(pairs.pair_class == "within_gut_vagina") & (pairs.shared_strain)].copy()
